...
void StartRTC(){
    ...
    for(int i = 0;i < 10;i++){
        Status=I2C_I2CMasterSendStart(RV8803_ADDR,I2C_I2C_READ_XFER_MODE,10);
        I2C_I2CMasterSendStop(10);
		if(Status==I2C_I2C_MSTR_NO_ERROR)   // If status==0 all is ok
            break;
    ...
    if (Status!=I2C_I2C_MSTR_NO_ERROR)
      while(1); /*manage here device not found */
    RTCdisableAllInterrupts();
    RTCsetCountdownTimerFrequency(COUNTDOWN_TIMER_FREQUENCY_64_HZ);
    RTCsetCountdownTimerClockTicks(128);
    RTCenableHardwareInterrupt(TIMER_INTERRUPT);
    RTCsetCountdownTimerEnable(1);// 1 is enable
}

CY_ISR(RTC_INT_Handler){
    RTC_INT_ClearInterrupt();
    if(RTCgetInterruptFlag(FLAG_TIMER)){
        RTCclearInterruptFlag(FLAG_TIMER);
      }
}
