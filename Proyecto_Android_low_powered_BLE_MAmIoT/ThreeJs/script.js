if ( ! Detector.webgl ) Detector.addGetWebGLMessage();
			...
			function init() {
				...
				loader.load("insole_left.dae", function ( geometry,materials ) {
					...
					LeftVerticesValue=		getOriginalVerticesValues(LeftXVerticesPositions,geometryModel);
					//We need distanto each FSR to calculate the influence factor.
					LeftDistanceToFSRs=		getDistanceToFSRs(LeftVerticesValue,LeftFSRLocation);
				...
				loader.load("insole_right.dae", function ( geometry,materials ) {
					...
					RightVerticesValue=		getOriginalVerticesValues(RightXVerticesPositions,geometryModel);
					//We need distanto each FSR to calculate the influence factor.
					RightDistanceToFSRs=		getDistanceToFSRs(RightVerticesValue,RightFSRLocation);
...
/* 			function animate() {
			...
			function searchVertex(geometry) {
				...
				var hightInterval=2;//take lateral vertex than have 2 mm from the floor.
				...
				for(i=0;i<normals.length;i=i+3){
					...
					else if(Math.abs(normals[i])>0.4 || Math.abs(normals[i+1]>0.4))//rule to detect lateral vertex with the normal vector.
					{ 
		...
		function getOriginalVerticesValues(XVerticesPositions,geometry){//Save original vertices positions.
			var TempOriginalVerticesValues=[];
			var positions= geometry.attributes.position.array;
			
			for(i=0,j=0;i<XVerticesPositions.length;i++,j=j+3){
				TempOriginalVerticesValues[j]=positions[XVerticesPositions[i]];
				TempOriginalVerticesValues[j+1]=positions[XVerticesPositions[i]+1];
				TempOriginalVerticesValues[j+2]=positions[XVerticesPositions[i]+2];
			}
			...
			function getDistanceToFSRs(verticesValue,FSRLocation) {
				//2 dimension array, for each vertex(X,Y,Z) we have 12 values in this array.
				var i, j, k;
				var TempDistanceToFSRs=[];
				var totalVertices=verticesValue.length/3;
				for(i=0,k=0;i<totalVertices;i++,k=k+3){
        ...
        for(i=0;i<xVertexPositions.length;i++){// For all vertices. i represents that vertex we are going to calculate.
			 ...
			 * 		15-25	->	0.9-0.5
			 * 		25-45	->	0.5-0
			 * maxF- [(Input Value-minD) / (maxD-minD) * (maxF-minF)]
			 */
				...
				if(tempDistance<45){
					...
					switch (true) {
						case (distanceToFSRs[i][j]>25):
							tempInfluenceFactor=0.5-((distanceToFSRs[i][j]-25)/20*0.5);
							break;
						case (distanceToFSRs[i][j]>15):
							tempInfluenceFactor=0.9-((distanceToFSRs[i][j]-15)/10*0.4);
							break;

						default:// distanceToFSRs[i][j]>0
							tempInfluenceFactor=1-(distanceToFSRs[i][j]/15*0.1);
					}
        ...
        for(i=0,k=0;i<verticesNumber;i++,k+=3){//For each vertex obtain the total influence from all FSRs.
				...
				if(influence>30){//The minimum value to start changing the vertex.
					...
					if(i<upVertices){//Only for surface vertex. i>upVertices are the lateral vertices.
						...
						colors[xVertexPositions[i]]=1; //No presion is white color
						colors[xVertexPositions[i]+1]=1; 
						colors[xVertexPositions[i]+2]=1;
					}else{
						// the lateral vertex has blue color.
