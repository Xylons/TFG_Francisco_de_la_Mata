...
public class ScanActivity extends AppCompatActivity {
    ...
    private List<BluetoothDevice> mBluetoothDevice = new ArrayList<>();
    private List<String> mBleName = new ArrayList<>();
    ...
    private Intent saveDataServiceIntent;
    ...
    protected void onResume() {
        ...
        boolean leftService= isServiceRunning("com.example.org.insole.LeftInsoleService");
        boolean rightService= isServiceRunning("com.example.org.insole.RightInsoleService");
        boolean saveDataService = isServiceRunning(SaveDataService.class.getName());

        String LeftAddress=preferences.getString(Tools.LEFT + Tools.INSOLE_ADDRESS, null);
        String RightAddress=preferences.getString(Tools.RIGHT + Tools.INSOLE_ADDRESS, null);
        ...
        saveDataServiceIntent = new Intent(this, SaveDataService.class);
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (preferences.getString(Tools.LEFT + Tools.INSOLE_ADDRESS, null) == null) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                        startForegroundService(RightInsoleServiceIntent) ;
                        startForegroundService(LeftInsoleServiceIntent) ;
                        startForegroundService(saveDataServiceIntent);
                    }else {
                        startService(RightInsoleServiceIntent);
                        startService(LeftInsoleServiceIntent);
                        startService(saveDataServiceIntent);
                    }
                    final Intent intent = new Intent(ScanActivity.this, DataViewActivityRAW.class);
                    scanLeDevice(false); // Stop scanning
                    startActivity(intent);
                }
            }
        });
        if(!rightService && !leftService && !saveDataService) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                startForegroundService(RightInsoleServiceIntent) ;
                startForegroundService(LeftInsoleServiceIntent) ;
                startForegroundService(saveDataServiceIntent);
    ...
    protected void onPause() {
        super.onPause();
        scanLeDevice(false);
        /// Esto tendría que moverse al ondestroy para que funcione despues de apagra pantalla
        ...
        switch (requestCode) {
            case PERMISSION_REQUEST_COARSE_LOCATION: {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    builder.setMessage("Since location access has not been granted, scanning will not work, and this app will close.");
